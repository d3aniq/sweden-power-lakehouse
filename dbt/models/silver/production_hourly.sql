...
{#
    Mimer's hourly values, normalised. Four things happen here:
    the Summa row goes, decimal comma becomes point, kWh becomes MWh,
    and republished hours are deduplicated to the latest version.
#}

with raw as (

    select *
    from {{ source('bronze', 'mimer_production') }}
